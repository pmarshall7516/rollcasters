...
async function readCatalog(client) {
  ...
  data.lootboxes = await q(`select id,name,description,closed_asset_path,open_asset_path,sell_currency_id,sell_value::text,sort_order,is_active,is_archived from public.lootboxes where is_active and not is_archived order by sort_order,id`);
  data.lootboxPoolEntries = (await q(`select id,lootbox_id,reward_type,target_category,target_id,min_amount,max_amount,probability,dupe_currency_id,dupe_currency_amount::text,sort_order from public.lootbox_pool_entries order by lootbox_id,sort_order,id`)).map((row) => numeric(row, ["probability"]));
