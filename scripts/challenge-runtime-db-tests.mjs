...
try {
  ...
  const fixture = await client.query(`
    select
      ...
      global_dungeon.parameters as global_dungeon_parameters
    ...
    cross join lateral (
      select challenge.id, challenge.parameters
      from public.release_collectible_challenges(public.current_game_catalog_release_id()) challenge
  ...
  await client.query(
    ...
    [userId, JSON.stringify({ won: true, dungeon_id: row.global_dungeon_parameters.dungeon_ids?.[0] ?? row.dungeon_id, dungeon_order: 10 })],
  );
