...
assert.match(closeHandler, /void completeShutdownCleanup\(\)/, "The native close handler must start shutdown cleanup without blocking the OS close.");
assert.doesNotMatch(closeHandler, /await completeShutdownCleanup\(\)/, "The native close handler must not hold the OS close request for a remote save.");
